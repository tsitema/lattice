    ...
    methods (Static)
        ...
        function pl=showNodes(nodes)
            ...
            if isempty(xps)||isempty(yps)
                ...
                if shownodes
                    pl=plot(gr, 'XData', xps, 'YData',yps,'NodeLabel',[nodes.ID]);%+0.1*rand(size(yps)));
                else
                    pl=plot(gr, 'XData', xps, 'YData',yps,'NodeLabel',[]);
                end
            end
            pl.MarkerSize = 9;
        ...
        function graphTimeAmp(soln, tstart, tend, ntime)
            %******PARAMETERS********
